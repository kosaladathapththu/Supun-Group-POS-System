...
$detail_q=$conn->query("SELECT t.transaction_id,t.receipt_number,t.customer_id,t.order_id,t.transaction_type,t.amount,t.remaining_amount,t.payment_method,t.reference_note,t.settlement_status,t.created_at,c.account_number,c.customer_name,c.phone,o.order_number,o.order_status,o.total_amount bill_total,o.advance_used,o.payment_method final_payment_method,o.paid_at,(SELECT COUNT(*) FROM advance_payment_transactions u WHERE u.parent_transaction_id=t.transaction_id AND u.transaction_type='sale_usage') usage_count FROM advance_payment_transactions t JOIN customer_accounts c ON c.customer_id=t.customer_id LEFT JOIN orders o ON o.order_id=t.order_id WHERE t.transaction_type='deposit' ORDER BY t.created_at DESC,t.transaction_id DESC");
...
<section class="card" style="margin-bottom:18px"><h2>Customer Balances</h2><form class="search"><input name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search name, account or phone"><button class="btn">Search</button></form><div class="table-wrap"><table><thead><tr><th>Account</th><th>Customer</th><th>Phone</th><th>Transactions</th><th>Available</th><?php if($can_edit_advances): ?><th>Action</th><?php endif; ?></tr></thead><tbody><?php while($c=$customers->fetch_assoc()): ?><tr><td><span class="chip"><?php echo htmlspecialchars($c['account_number']); ?></span></td><td><strong><?php echo htmlspecialchars($c['customer_name']); ?></strong></td><td><?php echo htmlspecialchars($c['phone']?:'-'); ?></td><td><?php echo (int)$c['transaction_count']; ?></td><td class="money">Rs. <?php echo number_format($c['advance_balance'],2); ?></td><?php if($can_edit_advances): ?><td><button type="button" class="btn" style="padding:7px 10px;background:#175cd3;white-space:nowrap" onclick='openCustomerEdit(<?php echo json_encode(["customer_id"=>(int)$c["customer_id"],"account_number"=>$c["account_number"],"customer_name"=>$c["customer_name"],"phone"=>$c["phone"],"address"=>$c["address"]],JSON_HEX_APOS|JSON_HEX_QUOT); ?>)'><i class="fa-solid fa-pen"></i> Edit</button></td><?php endif; ?></tr><?php endwhile; ?></tbody></table></div></section>
