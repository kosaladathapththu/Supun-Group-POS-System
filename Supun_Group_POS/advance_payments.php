...
$detail_q=$conn->query("SELECT t.transaction_id,t.receipt_number,t.customer_id,t.order_id,t.transaction_type,t.amount,t.remaining_amount,t.payment_method,t.created_at,c.account_number,c.customer_name,c.phone,o.order_number,o.order_status FROM advance_payment_transactions t JOIN customer_accounts c ON c.customer_id=t.customer_id LEFT JOIN orders o ON o.order_id=t.order_id WHERE t.transaction_type='deposit' ORDER BY t.created_at DESC,t.transaction_id DESC");
...
function renderCustomerPaymentSummary(){
 ...
 customerPaymentDetails.forEach(payment=>{const key=String(payment.customer_id);if(!grouped.has(key))grouped.set(key,{customer_id:key,account_number:payment.account_number,customer_name:payment.customer_name,phone:payment.phone,payments:[],received:0,available:0,orders:new Set()});const customer=grouped.get(key);customer.payments.push(payment);customer.received+=Number(payment.amount||0);customer.available+=Number(payment.remaining_amount||0);if(payment.order_number)customer.orders.add(payment.order_number)});
 window.customerPaymentGroups=[...grouped.values()];
 const rows=window.customerPaymentGroups.length?window.customerPaymentGroups.map((customer,index)=>`<tr><td><span class="chip">${safeText(customer.account_number)}</span></td><td><strong>${safeText(customer.customer_name)}</strong><br><small>${safeText(customer.phone||'-')}</small></td><td>${customer.payments.length}</td><td>${customer.orders.size}</td><td>Rs. ${Number(customer.received).toLocaleString('en-LK',{minimumFractionDigits:2})}</td><td class="money">Rs. ${Number(customer.available).toLocaleString('en-LK',{minimumFractionDigits:2})}</td><td><button type="button" class="btn view-payments-btn" data-index="${index}" style="padding:8px 11px;white-space:nowrap">View All Payments</button></td></tr>`).join(''):'<tr><td colspan="7" style="text-align:center;color:#667085">No customer payments found.</td></tr>';
 card.innerHTML=`<h2><i class="fa-solid fa-users"></i> Customer Payment Accounts</h2><div class="hint" style="margin-bottom:12px">One row per customer. Select View All Payments to see installments, orders, methods and receipts.</div><div class="table-wrap"><table><thead><tr><th>Account</th><th>Customer</th><th>Payments</th><th>Orders</th><th>Total Received</th><th>Available Credit</th><th>Action</th></tr></thead><tbody>${rows}</tbody></table></div>`;
