...
$message = isset($_GET['saved']) ? 'Advance payment saved successfully. The customer balance has been updated.' : '';
$message_type = 'success';
$methods = ['Cash','Card','QR','Bank Transfer','Cheque'];
$can_edit_advances = in_array($_SESSION['role'] ?? '', ['admin', 'accountant'], true);
$installment_only = defined('INSTALLMENT_ONLY_VIEW') && INSTALLMENT_ONLY_VIEW;

if (isset($_POST['edit_customer'])) {
    if (!$can_edit_advances) { http_response_code(403); $message='You do not have permission to edit customer accounts.'; $message_type='error'; }
    else {
        $customer_id=(int)($_POST['edit_customer_id']??0); $name=trim($_POST['edit_customer_name']??'');
        $phone=trim($_POST['edit_customer_phone']??''); $address=trim($_POST['edit_customer_address']??'');
        if ($customer_id<=0 || $name==='') { $message='Enter a valid customer name.'; $message_type='error'; }
        else {
            $stmt=$conn->prepare('UPDATE customer_accounts SET customer_name=?,phone=?,address=? WHERE customer_id=?');
            $stmt->bind_param('sssi',$name,$phone,$address,$customer_id);
            if ($stmt->execute() && $stmt->affected_rows>=0) $message='Customer details updated successfully.';
            else { $message='Could not update customer details.'; $message_type='error'; }
            $stmt->close();
        }
    }
}
...
if (isset($_POST['edit_advance'])) {
    if (!$can_edit_advances) { http_response_code(403); $message='You do not have permission to edit advance payments.'; $message_type='error'; }
    else {
        ...
        if ($transaction_id<=0 || $new_amount<=0) { $message='Enter a valid advance amount.'; $message_type='error'; }
        else {
            ...
            try {
                ...
                $old_amount=round((float)$payment['amount'],2); $difference=round($new_amount-$old_amount,2); $customer_id=(int)$payment['customer_id'];
...
if (isset($_POST['cancel_order_refund'])) {
    if (!$can_edit_advances) { http_response_code(403); $message='You do not have permission to cancel bills.'; $message_type='error'; }
    else {
        $order_id=(int)($_POST['cancel_order_id']??0); $refund_method=trim($_POST['refund_method']??'Cash'); $reason=trim($_POST['cancellation_reason']??'');
        if (!in_array($refund_method,$methods,true)) $refund_method='Cash';
        if ($order_id<=0 || $reason==='') { $message='Select a valid bill and enter the cancellation reason.'; $message_type='error'; }
        else {
            $conn->begin_transaction();
            try {
                $stmt=$conn->prepare("SELECT o.order_id,o.order_number,o.customer_id,o.order_status,o.payment_status,o.total_amount FROM orders o WHERE o.order_id=? AND o.order_status IN ('open','paid') FOR UPDATE");
                ...
                $items=$conn->query("SELECT oi.product_id,SUM(oi.quantity) quantity,MAX(oi.cost_price) unit_cost FROM order_items oi WHERE oi.order_id=$order_id AND oi.product_id IS NOT NULL GROUP BY oi.product_id FOR UPDATE");
                $uid=(int)$_SESSION['user_id'];
                while($items && $item=$items->fetch_assoc()) {
                    $product_id=(int)$item['product_id']; $quantity=(float)$item['quantity']; $unit_cost=(float)$item['unit_cost'];
                    $pq=$conn->query("SELECT stock_qty FROM products WHERE product_id=$product_id FOR UPDATE"); $product=$pq?$pq->fetch_assoc():null;
                    if(!$product) continue; $before=(float)$product['stock_qty']; $after=$before+$quantity; $total_cost=$quantity*$unit_cost;
                    $stmt=$conn->prepare('UPDATE products SET stock_qty=? WHERE product_id=?'); $stmt->bind_param('di',$after,$product_id); if(!$stmt->execute()) throw new Exception($stmt->error); $stmt->close();
                    $note='Stock restored from cancelled bill '.$order['order_number'];
                    $stmt=$conn->prepare("INSERT INTO stock_adjustments (product_id,user_id,adjustment_type,quantity,stock_before,stock_after,unit_cost,total_cost,note,invoice_status) VALUES (?,?,'stock_in',?,?,?,?,?,?,?,'not_required')");
                    $stmt->bind_param('iiddddds',$product_id,$uid,$quantity,$before,$after,$unit_cost,$total_cost,$note); if(!$stmt->execute()) throw new Exception($stmt->error); $stmt->close();
                }

                $receipt=nextAdvanceReceipt($conn); $reference='Bill cancelled: '.$reason;
                $stmt=$conn->prepare("INSERT INTO advance_payment_transactions (receipt_number,customer_id,order_id,transaction_type,amount,remaining_amount,settlement_status,payment_method,reference_note,created_by) VALUES (?,?,?,'refund',?,0,'settled',?,?,?)");
                $stmt->bind_param('siidssi',$receipt,$customer_id,$order_id,$refund_amount,$refund_method,$reference,$uid); if(!$stmt->execute()) throw new Exception($stmt->error); $refund_id=$conn->insert_id; $stmt->close();
                $stmt=$conn->prepare("UPDATE orders SET order_status='cancelled',payment_status='cancelled',balance=0 WHERE order_id=?"); $stmt->bind_param('i',$order_id); if(!$stmt->execute()) throw new Exception($stmt->error); $stmt->close();
                $stmt=$conn->prepare('INSERT INTO order_cancellations (order_id,refund_transaction_id,refund_amount,refund_method,cancellation_reason,stock_restored,cancelled_by) VALUES (?,?,?,?,?,1,?)');
                $stmt->bind_param('iidssi',$order_id,$refund_id,$refund_amount,$refund_method,$reason,$uid); if(!$stmt->execute()) throw new Exception($stmt->error); $stmt->close();
                $conn->commit(); header('Location: print_advance.php?transaction_id='.$refund_id); exit;
            } catch(Throwable $e) { $conn->rollback(); $message='Could not cancel bill: '.$e->getMessage(); $message_type='error'; }
        }
    }
}
if (isset($_POST['complete_order'])) {
    ...
    try {
        ...
        header("Location: print_bill.php?order_id=$order_id&from=advance"); exit;
    } catch (Throwable $e) {
...
if (isset($_POST['save_advance'])) {
    ...
    if ($amount <= 0 || ($customer_id <= 0 && $name === '')) {
        ...
        try {
            ...
            header("Location: advance_payments.php?saved=1&transaction_id=$advance_transaction_id"); exit;
        } catch (Throwable $e) {
...
$customers = $conn->query("SELECT c.*,(SELECT COALESCE(SUM(d.remaining_amount),0) FROM advance_payment_transactions d WHERE d.customer_id=c.customer_id AND d.transaction_type='deposit' AND d.order_id IS NULL AND d.remaining_amount>0) available_credit,COUNT(t.transaction_id) transaction_count FROM customer_accounts c LEFT JOIN advance_payment_transactions t ON t.customer_id=c.customer_id WHERE $where GROUP BY c.customer_id ORDER BY c.customer_name");
$select_customers = $conn->query("SELECT c.customer_id,c.account_number,c.customer_name,c.phone,(SELECT COALESCE(SUM(d.remaining_amount),0) FROM advance_payment_transactions d WHERE d.customer_id=c.customer_id AND d.transaction_type='deposit' AND d.order_id IS NULL AND d.remaining_amount>0) available_credit FROM customer_accounts c WHERE c.status=1 ORDER BY c.customer_name");
...
while($detail_q && $detail=$detail_q->fetch_assoc()) { $detail['can_edit']=$can_edit_advances && (int)$detail['usage_count']===0 && abs((float)$detail['amount']-(float)$detail['remaining_amount'])<0.005 && $detail['order_status']!=='paid'; $customer_payment_details[]=$detail; }
$account_credit_usages=[];
$usage_q=$conn->query("SELECT u.customer_id,u.order_id,COALESCE(o.order_number,CONCAT('Order #',u.order_id)) order_number,o.total_amount bill_total,o.order_status,COALESCE(o.paid_at,o.created_at,u.created_at) used_at,SUM(u.amount) amount_used,(SELECT GROUP_CONCAT(CONCAT(COALESCE(p.product_name,oi.custom_item_name,'Item'),' x',oi.quantity) ORDER BY oi.order_item_id SEPARATOR ', ') FROM order_items oi LEFT JOIN products p ON p.product_id=oi.product_id WHERE oi.order_id=u.order_id) items FROM advance_payment_transactions u JOIN advance_payment_transactions d ON d.transaction_id=u.parent_transaction_id LEFT JOIN orders o ON o.order_id=u.order_id WHERE u.transaction_type='sale_usage' AND d.transaction_type='deposit' AND d.order_id IS NULL GROUP BY u.customer_id,u.order_id,o.order_number,o.total_amount,o.order_status,o.paid_at,o.created_at,u.created_at ORDER BY used_at DESC,u.order_id DESC");
while($usage_q && $usage=$usage_q->fetch_assoc()) $account_credit_usages[]=$usage;
$summary = $conn->query("SELECT COUNT(*) customers,(SELECT COALESCE(SUM(t.remaining_amount),0) FROM advance_payment_transactions t JOIN customer_accounts ac ON ac.customer_id=t.customer_id WHERE t.transaction_type='deposit' AND t.order_id IS NULL AND t.remaining_amount>0 AND ac.status=1) balance FROM customer_accounts WHERE status=1")->fetch_assoc();
...
<script>document.body.classList.toggle('installment-only',<?php echo $installment_only?'true':'false'; ?>)</script>
<?php if(!isset($_GET['embedded'])): ?><div class="top"><strong><i class="fa-solid <?php echo $installment_only?'fa-file-invoice-dollar':'fa-wallet'; ?>"></i> <?php echo $installment_only?'Order Installments':'Account Credit'; ?></strong><div><a href="pos.php"><i class="fa-solid fa-arrow-left"></i> Back to POS</a><?php if(in_array($_SESSION['role']??'', ['admin','accountant'],true)): ?> &nbsp; <a href="dashboard.php"><i class="fa-solid fa-gauge-high"></i> Dashboard</a><?php endif; ?></div></div><?php endif; ?>
<main class="wrap"><div class="head"><div><h1><?php echo $installment_only?'Order Installment Payments':'Customer Account Credit'; ?></h1><div class="hint"><?php echo $installment_only?'Payments tied to a selected order. The remaining amount is the unpaid bill balance.':'Flexible customer money held for a future purchase and not tied to an order.'; ?></div></div><div class="mode-switch"><a class="btn <?php echo !$installment_only?'active':''; ?>" href="advance_payments.php<?php echo isset($_GET['embedded'])?'?embedded=1':''; ?>"><i class="fa-solid fa-wallet"></i> Account Credit</a><a class="btn <?php echo $installment_only?'active':''; ?>" href="installment_payments.php<?php echo isset($_GET['embedded'])?'?embedded=1':''; ?>"><i class="fa-solid fa-file-invoice-dollar"></i> Installments</a></div></div>
...
<div class="field"><label>Account credit amount *</label><input type="number" min="0.01" step="0.01" name="amount" required></div><div class="field"><label>Received by</label><select name="payment_method"><?php foreach($methods as $m): ?><option><?php echo $m; ?></option><?php endforeach; ?></select></div><div class="field"><label>Reference / purpose</label><textarea name="reference_note" placeholder="Example: Customer will choose products tomorrow"></textarea></div><div class="field"><label>Receipt option</label><select name="issue_receipt"><option value="1">Save and open receipt for printing</option><option value="0">Save only — no receipt printing now</option></select><div class="hint" style="margin-top:5px">A receipt number is always recorded for audit history. You can print it later from the payment list.</div></div><button class="btn" name="save_advance" style="width:100%"><i class="fa-solid fa-floppy-disk"></i> Save Account Credit</button></form></section>
<div><div class="stats"><div class="stat"><strong><?php echo number_format($summary['customers']); ?></strong><span>Customer accounts</span></div><div class="stat"><strong>Rs. <?php echo number_format($summary['balance'],2); ?></strong><span>Account credit liability</span></div><div class="stat"><strong><?php echo number_format((int)$open_summary['open_items']); ?></strong><span>Open installments</span></div><div class="stat"><strong><?php echo number_format((int)$open_summary['overdue']); ?></strong><span>Past follow-up date</span></div></div>
<section class="card" style="margin-bottom:18px"><h2>Customer Account Credit</h2><form class="search"><input name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search name, account or phone"><button class="btn">Search</button></form><div class="hint" style="margin-bottom:10px">Only flexible money not linked to an order is shown here. Order installments are shown separately below.</div><div class="table-wrap"><table><thead><tr><th>Account</th><th>Customer</th><th>Phone</th><th>Transactions</th><th>Available Credit</th><?php if($can_edit_advances): ?><th>Action</th><?php endif; ?></tr></thead><tbody><?php while($c=$customers->fetch_assoc()): ?><tr><td><span class="chip"><?php echo htmlspecialchars($c['account_number']); ?></span></td><td><strong><?php echo htmlspecialchars($c['customer_name']); ?></strong></td><td><?php echo htmlspecialchars($c['phone']?:'-'); ?></td><td><?php echo (int)$c['transaction_count']; ?></td><td class="money">Rs. <?php echo number_format($c['available_credit'],2); ?></td><?php if($can_edit_advances): ?><td><button type="button" class="btn" style="padding:7px 10px;background:#175cd3;white-space:nowrap" onclick='openCustomerEdit(<?php echo json_encode(["customer_id"=>(int)$c["customer_id"],"account_number"=>$c["account_number"],"customer_name"=>$c["customer_name"],"phone"=>$c["phone"],"address"=>$c["address"]],JSON_HEX_APOS|JSON_HEX_QUOT); ?>)'><i class="fa-solid fa-pen"></i> Edit</button></td><?php endif; ?></tr><?php endwhile; ?></tbody></table></div></section>
...
const accountCreditUsages=<?php echo json_encode($account_credit_usages,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES); ?>;
