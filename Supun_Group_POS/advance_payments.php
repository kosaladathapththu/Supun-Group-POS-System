...
<section class="card"><h2>Individual Advance Payments &amp; Settlements</h2><div class="hint" style="margin-bottom:12px;">Each deposit is followed separately. The follow-up date defaults to the next day. Using it in a sale settles it automatically; Settle / Refund returns any unused amount and closes it.</div><div class="table-wrap"><table><thead><tr><th>Date</th><th>Receipt / Customer</th><th>Type</th><th>Original</th><th>Remaining</th><th>Follow-up</th><th>Status</th><th>Actions</th></tr></thead><tbody><?php while($t=$transactions->fetch_assoc()): $is_open_deposit=$t['transaction_type']==='deposit' && $t['settlement_status']!=='settled' && (float)$t['remaining_amount']>0; ?><tr><td><?php echo date('d M Y H:i',strtotime($t['created_at'])); ?></td><td><strong><?php echo htmlspecialchars($t['receipt_number']); ?></strong><br><?php echo htmlspecialchars($t['customer_name']); ?> <small><?php echo htmlspecialchars($t['account_number']); ?></small></td><td class="<?php echo htmlspecialchars($t['transaction_type']); ?>"><?php echo ucwords(str_replace('_',' ',$t['transaction_type'])); ?><br><small><?php echo htmlspecialchars($t['payment_method']); ?></small></td><td><strong>Rs. <?php echo number_format($t['amount'],2); ?></strong></td><td><?php echo $t['transaction_type']==='deposit'?'Rs. '.number_format($t['remaining_amount'],2):'-'; ?></td><td><?php echo $t['settlement_due_date']?date('d M Y',strtotime($t['settlement_due_date'])):'-'; ?></td><td><span class="chip"><?php echo ucfirst($t['settlement_status']); ?></span></td><td><a class="btn" style="padding:7px 9px;white-space:nowrap;" href="print_advance.php?transaction_id=<?php echo (int)$t['transaction_id']; ?>">Print</a><?php if($is_open_deposit): ?><form method="post" style="margin-top:5px;display:flex;gap:4px;" onsubmit="return confirm('Refund the unused Rs. <?php echo number_format($t['remaining_amount'],2); ?> and settle this advance?');"><input type="hidden" name="deposit_id" value="<?php echo (int)$t['transaction_id']; ?>"><select name="settle_method" style="max-width:82px;border:1px solid #ccc;border-radius:6px;"><option>Cash</option><option>Card</option><option>QR</option><option>Bank Transfer</option></select><button class="btn" name="settle_deposit" style="padding:7px 9px;white-space:nowrap;background:#b42318;">Settle / Refund</button></form><?php endif; ?></td></tr><?php endwhile; ?></tbody></table></div></section></div></div></main>
