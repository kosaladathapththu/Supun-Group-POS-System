...
function renderCustomerPaymentSummary(){
 ...
 customerPaymentDetails.forEach(payment=>{const key=String(payment.customer_id)+'|'+String(payment.order_id||'unlinked');if(!grouped.has(key))grouped.set(key,{customer_id:String(payment.customer_id),order_id:Number(payment.order_id||0),order_number:payment.order_number||'Not linked',order_status:payment.order_status||'open',account_number:payment.account_number,customer_name:payment.customer_name,phone:payment.phone,payments:[],received:0,available:0});const order=grouped.get(key);order.payments.push(payment);order.received+=Number(payment.amount||0);order.available+=Number(payment.remaining_amount||0);if(payment.order_status)order.order_status=payment.order_status});
