...
document.addEventListener('click',event=>{const link=event.target.closest('#customerHistoryModal a[href*="transaction_id=-"]');if(!link)return;event.preventDefault();const orderId=Math.abs(Number(new URL(link.href).searchParams.get('transaction_id')));if(orderId)window.location.href='print_bill.php?order_id='+orderId});
