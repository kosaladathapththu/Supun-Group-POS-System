...
function openSettlement(data){settlementDue=Number(data.due||0);document.getElementById('settleOrderId').value=data.order_id;document.getElementById('settleOrder').value=data.order;document.getElementById('settleCustomer').textContent=data.customer+(data.phone?' · '+data.phone:'');document.getElementById('settleTotal').textContent=money(data.total);document.getElementById('settleAdvance').textContent=money(data.advance);document.getElementById('settleDue').textContent=money(data.due);document.getElementById('settleMethod').value='Cash';document.getElementById('settleReceived').value=Number(data.due).toFixed(2);updateSettlementMethod();document.getElementById('settlementModal').classList.add('open')}
function closeSettlement(){document.getElementById('settlementModal').classList.remove('open')}
function updateSettlementMethod(){const cash=document.getElementById('settleMethod').value==='Cash',input=document.getElementById('settleReceived');input.readOnly=!cash;document.getElementById('receivedLabel').textContent=cash?'Cash received *':'Amount charged';if(!cash)input.value=settlementDue.toFixed(2);updateChange()}
function updateChange(){const received=Number(document.getElementById('settleReceived').value||0);document.getElementById('settleChange').textContent=money(Math.max(0,received-settlementDue));document.getElementById('changeBox').style.display=document.getElementById('settleMethod').value==='Cash'?'block':'none'}
