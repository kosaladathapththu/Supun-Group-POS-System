...
function updateSettlementMethod(){document.getElementById('settleReceived').readOnly=false;document.getElementById('receivedLabel').textContent='Amount paying now *';updateChange()}
function updateChange(){const received=Number(document.getElementById('settleReceived').value||0),cash=document.getElementById('settleMethod').value==='Cash',isFull=received+0.0001>=settlementDue;document.getElementById('settleChange').textContent=money(cash&&isFull?Math.max(0,received-settlementDue):0);document.getElementById('changeBox').style.display=cash&&isFull?'block':'none';const button=document.querySelector('#settlementForm button[name="complete_order"]');button.innerHTML=isFull?'Complete &amp; Print Final Bill':'Save Installment &amp; Print Receipt';button.onclick=()=>confirm(isFull?'Complete this order and print the final bill?':'Save this installment and keep the remaining balance open?')}
