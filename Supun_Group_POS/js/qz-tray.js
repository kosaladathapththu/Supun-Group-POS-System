...
var qz = (function () {
  ...
  var _qz = {
      ...
      setup: {
        ...
        findConnection: function (config, resolve, reject) {
          ...
          var deeper = function () {
            ...
            if (
              (config.usingSecure &&
                config.port.portIndex >= config.port.secure.length) ||
              (!config.usingSecure &&
                config.port.portIndex >= config.port.insecure.length)
            ) {
          ...
          if (_qz.websocket.connection != null) {
            ...
            _qz.websocket.connection.onopen = function (evt) {
              if (!_qz.websocket.connection.established) {
                _qz.log.trace(evt);
                _qz.log.info(
                  "Established connection with QZ Tray on " + address,
                );
            ...
            for (var uid in _qz.websocket.pendingCalls) {
              if (_qz.websocket.pendingCalls.hasOwnProperty(uid)) {
                _qz.websocket.pendingCalls[uid].reject(
                  new Error("Connection closed before response received"),
                );
              }
            }
                ...
                for (
                  ...
                  try {
                    if (
                      ...
                      _qz.websocket.connection.semver[i] =
                        -_qz.websocket.connection.semver[i].replace(/\D/g, "");
                      continue;
                    }
                    _qz.websocket.connection.semver[i] = parseInt(
                      _qz.websocket.connection.semver[i],
                    );
                  } catch (ignore) {}
    ...
    security: {
      ...
      callCert: function () {
        if (typeof _qz.security.certHandler.then === "function") {
          ...
          _qz.security.certHandler.constructor.name === "AsyncFunction"
        ) {
      ...
      callSign: function (toSign) {
        if (
          _qz.security.signatureFactory.constructor.name === "AsyncFunction"
        ) {
          //use directly
        ...
        for (var i = 0; i < data.length; i++) {
          if (data[i].constructor === Object) {
            ...
            if (
              ...
              data[i].type &&
              ((["PIXEL", "IMAGE", "PDF"].indexOf(data[i].type.toUpperCase()) >
                -1 &&
                !data[i].format) ||
          ...
          for (var i = 0; i < printData.length; i++) {
            if (printData[i].constructor === Object) {
              if (
                ...
                if (
                  printData[i].flavor &&
                  printData[i].flavor.toUpperCase() === "BASE64"
                ) {
                  //special case for raw base64 images
                  printData[i].data =
                    "data:image/compat;base64," + printData[i].data;
                }
              ...
              if (
                (printData[i].type &&
                  printData[i].type.toUpperCase() === "RAW") ||
                (printData[i].format &&
                  printData[i].format.toUpperCase() === "COMMAND")
              ) {
    ...
    SHA: {
      ...
      _sig0: function (x) {
        return (
          _qz.SHA._rotr(2, x) ^ _qz.SHA._rotr(13, x) ^ _qz.SHA._rotr(22, x)
        );
      },
      _sig1: function (x) {
        return (
          _qz.SHA._rotr(6, x) ^ _qz.SHA._rotr(11, x) ^ _qz.SHA._rotr(25, x)
        );
      },
      _dev0: function (x) {
    ...
    this.setPrinter = function (newPrinter) {
      ...
      if (newPrinter && newPrinter.file) {
        ...
        if (newPrinter.file.indexOf("\\\\") != 0) {
          _qz.log.warn(
            "Printing to file is deprecated.  See https://github.com/qzind/tray/issues/730",
          );
        }
      }
      ...
      connect: function (options) {
        return _qz.tools.promise(function (resolve, reject) {
          if (_qz.tools.isActive()) {
            ...
            reject(
              new Error("The current connection attempt has not returned yet"),
            );
            return;
          }
