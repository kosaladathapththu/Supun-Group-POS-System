...
if (isset($_GET["imported"])) {
    $msg =
        (int) $_GET["imported"] .
        " inventory lines processed successfully. New items were created and existing item stock was increased.";
    $msg_type = "success";
}
...
if (isset($_POST["add_product"])) {
    ...
    $supplier_id_input = (int) ($_POST["supplier_id"] ?? 0);
    $supplier_code = trim($_POST["supplier_code"] ?? "");
    $supplier_name = trim($_POST["supplier_name"] ?? "");
    $supplier_phone = trim($_POST["supplier_phone"] ?? "");
    $supplier_invoice = trim($_POST["supplier_invoice"] ?? "");
    ...
    $min_qty = max(1, (int) ($_POST["wholesale_min_qty"] ?? 1));
    $stock = max(0, (float) ($_POST["stock_qty"] ?? 0));
    ...
    if (
        $cat_name === "" ||
        $prod_name === "" ||
        (!$invoice_pending && $supplier_name === "") ||
        $price <= 0 ||
        $stock <= 0
    ) {
        $msg =
            "Enter the category, product name, retail price and purchase quantity" .
            ($invoice_pending ? "." : ", including the supplier.");
        $msg_type = "error";
    } else {
        ...
        try {
            ...
            $find->bind_param("s", $cat_name);
            $find->execute();
            $cat = $find->get_result()->fetch_assoc();
            $find->close();
            if ($cat) {
            ...
            if ($invoice_pending) {
                ...
                $pending_note = "New product received before supplier invoice";
                $invoice_status = "pending";
                $log = $conn->prepare(
                    "INSERT INTO stock_adjustments(product_id,user_id,adjustment_type,quantity,stock_before,stock_after,unit_cost,total_cost,note,invoice_status) VALUES(?,?,'stock_in',?,0,?,0,0,?,?)",
                );
                $log->bind_param(
                    "iiddss",
                    $product_id,
                ...
                $conn->commit();
                $msg =
                    "Product and physical stock added. Its supplier invoice is pending and can be completed below.";
                $msg_type = "success";
            } else {
                $supplier = null;
                if ($supplier_id_input > 0) {
                    $find = $conn->prepare(
                        "SELECT supplier_id FROM suppliers WHERE supplier_id=? AND status=1 LIMIT 1",
                    );
                    $find->bind_param("i", $supplier_id_input);
                    $find->execute();
                ...
                if ($supplier_code !== "") {
                    ...
                    $find->bind_param("s", $supplier_code);
                    $find->execute();
                    $supplier = $find->get_result()->fetch_assoc();
                    $find->close();
                }
                if (!$supplier) {
                    ...
                    $find->bind_param("s", $supplier_name);
                    $find->execute();
                    $supplier = $find->get_result()->fetch_assoc();
                    $find->close();
                }
                ...
                $uid = (int) $_SESSION["user_id"];
                $notes = "Created from manual unified inventory entry";
                $add = $conn->prepare(
                ...
                $purchase_number =
                    "PUR-" .
                    str_pad((string) $purchase_id, 6, "0", STR_PAD_LEFT);
                $number = $conn->prepare(
                    "UPDATE purchases SET purchase_number=? WHERE purchase_id=?",
                );
                $number->bind_param("si", $purchase_number, $purchase_id);
                $number->execute();
                $number->close();
                $insert = $conn->prepare(
                    "INSERT INTO products(category_id,sku,barcode,serial_no,product_name,brand,unit,cost_price,price,wholesale_price,wholesale_min_qty,stock_qty,reorder_level,status) VALUES(?,NULLIF(?,''),NULLIF(?,''),NULLIF(?,''),?,NULLIF(?,''),?,?,?,?,?,?,?,?)",
                ...
                $finish->close();
                $note =
                    "Received from manual inventory entry - " .
                    $purchase_number;
                $log = $conn->prepare(
                    "INSERT INTO stock_adjustments(product_id,user_id,adjustment_type,quantity,stock_before,stock_after,unit_cost,total_cost,note) VALUES(?,?,'stock_in',?,0,?,?,?,?)",
                );
                $log->bind_param(
                    "iidddds",
                    $product_id,
                ...
                $log->close();
                $conn->commit();
                $msg = "Product, supplier purchase and stock added successfully ($purchase_number).";
                $msg_type = "success";
            }
        } catch (Throwable $e) {
            $conn->rollback();
            $msg =
                $e->getCode() === 1062
                    ? "The item code, barcode or serial number already exists."
                    : $e->getMessage();
            $msg_type = "error";
        }
    }
...
if (isset($_POST["edit_product"])) {
    ...
    $sku = trim($conn->real_escape_string($_POST["sku"] ?? ""));
    $barcode = trim($conn->real_escape_string($_POST["barcode"] ?? ""));
    $serial = trim($conn->real_escape_string($_POST["serial_no"] ?? ""));
    $brand = trim($conn->real_escape_string($_POST["brand"] ?? ""));
    $cost = (float) ($_POST["cost_price"] ?? 0);
    $wholesale = (float) ($_POST["wholesale_price"] ?? 0);
    $min_qty = max(1, (int) ($_POST["wholesale_min_qty"] ?? 10));
    $stock = max(0, (float) ($_POST["stock_qty"] ?? 0));
    $reorder = max(0, (float) ($_POST["reorder_level"] ?? 5));
    $unit = trim($conn->real_escape_string($_POST["unit"] ?? "pcs"));
    $status = (int) ($_POST["status"] ?? 1);

    if ($cat_name !== "" && $prod_name !== "" && $price > 0) {
        $cat_q = $conn->query(
            "SELECT category_id FROM categories WHERE category_name='$cat_name' LIMIT 1",
        );
        if ($cat_q->num_rows > 0) {
            $category_id = $cat_q->fetch_assoc()["category_id"];
        } else {
            $conn->query(
                "INSERT INTO categories (category_name, status) VALUES ('$cat_name', 1)",
            );
            $category_id = $conn->insert_id;
        }
        $sku_sql = $sku === "" ? "NULL" : "'$sku'";
        $barcode_sql = $barcode === "" ? "NULL" : "'$barcode'";
        $serial_sql = $serial === "" ? "NULL" : "'$serial'";
        $brand_sql = $brand === "" ? "NULL" : "'$brand'";
        $conn->query(
            "UPDATE products SET category_id=$category_id,sku=$sku_sql,barcode=$barcode_sql,serial_no=$serial_sql,brand=$brand_sql,product_name='$prod_name',unit='$unit',cost_price=$cost,price=$price,wholesale_price=$wholesale,wholesale_min_qty=$min_qty,reorder_level=$reorder,status=$status WHERE product_id=$id",
        );
        $msg = "Product updated.";
        $msg_type = "success";
    }
...
if (isset($_POST["adjust_stock"])) {
    ...
    $invoice_pending =
        $action === "stock_in" && isset($_POST["invoice_pending"]);
    ...
    if (
        ...
        try {
            $lock = $conn->prepare(
                "SELECT stock_qty,cost_price FROM products WHERE product_id=? FOR UPDATE",
            );
            $lock->bind_param("i", $product_id);
            $lock->execute();
            $stock_row = $lock->get_result()->fetch_assoc();
            $lock->close();
            if (!$stock_row) {
            ...
            $unit_cost =
                $action === "stock_in" && !$invoice_pending
                    ? ($purchase_unit_cost > 0
                        ? $purchase_unit_cost
                        : (float) $stock_row["cost_price"])
                    : 0;
            $total_cost =
                $action === "stock_in" && !$invoice_pending
                    ? $unit_cost * $quantity
                    : 0;
            $update = $conn->prepare(
                "UPDATE products SET stock_qty=?,cost_price=CASE WHEN ?='stock_in' AND ?>0 THEN ? ELSE cost_price END WHERE product_id=?",
            );
            $update->bind_param(
                "dsddi",
                $after,
            ...
            $log = $conn->prepare(
                "INSERT INTO stock_adjustments (product_id,user_id,adjustment_type,quantity,stock_before,stock_after,unit_cost,total_cost,note,invoice_status) VALUES (?,?,?,?,?,?,?,?,?,?)",
            );
            $log->bind_param(
                "iisdddddss",
                $product_id,
            ...
            $conn->commit();
            $msg = $invoice_pending
                ? "Stock received. Supplier invoice is pending and can be completed below without adding stock again."
        ...
        $msg = "Select a product and enter a valid quantity.";
        $msg_type = "error";
    }
}
...
if (isset($_POST["finalize_pending_invoice"])) {
    ...
    $supplier_id_input = (int) ($_POST["pending_supplier_id"] ?? 0);
    $supplier_code = trim($_POST["pending_supplier_code"] ?? "");
    $supplier_name = trim($_POST["pending_supplier_name"] ?? "");
    $supplier_phone = trim($_POST["pending_supplier_phone"] ?? "");
    $supplier_invoice = trim($_POST["pending_supplier_invoice"] ?? "");
    $invoice_date = trim($_POST["pending_invoice_date"] ?? "") ?: date("Y-m-d");
    $unit_cost = max(0, (float) ($_POST["pending_unit_cost"] ?? 0));
    $payment_method = trim($_POST["pending_payment_method"] ?? "Cash");
    $is_paid = isset($_POST["pending_paid"]);
    if (
        $adjustment_id <= 0 ||
        $supplier_name === "" ||
        $supplier_invoice === "" ||
        ...
        $msg = "Enter the supplier, invoice number and actual cost per item.";
        $msg_type = "error";
    } else {
        $conn->begin_transaction();
        try {
            $lock = $conn->prepare(
                "SELECT sa.*,p.product_name FROM stock_adjustments sa JOIN products p ON p.product_id=sa.product_id WHERE sa.adjustment_id=? AND sa.adjustment_type='stock_in' AND sa.invoice_status='pending' FOR UPDATE",
            );
            $lock->bind_param("i", $adjustment_id);
            $lock->execute();
            $pending = $lock->get_result()->fetch_assoc();
            $lock->close();
            if (!$pending) {
                throw new RuntimeException(
                    "This pending stock invoice was already completed or no longer exists.",
                );
            }
            $supplier = null;
            if ($supplier_id_input > 0) {
            ...
            $uid = (int) $_SESSION["user_id"];
            $paid_amount = $is_paid ? $total : 0;
            $payment_status = $is_paid ? "paid" : "unpaid";
            $notes =
                "Invoice completed for stock received earlier. Adjustment #" .
                $adjustment_id;
            $purchase = $conn->prepare(
                "INSERT INTO purchases(supplier_id,supplier_invoice,purchase_date,status,payment_status,subtotal,total_amount,paid_amount,notes,created_by,received_by,received_at) VALUES(?,? ,?,'received',?,?,?,?,?,?,?,NOW())",
            );
            $purchase->bind_param(
                "isssdddsii",
                $supplier_id,
            ...
            $purchase->close();
            $purchase_number =
                "PUR-" . str_pad((string) $purchase_id, 6, "0", STR_PAD_LEFT);
            $number = $conn->prepare(
                "UPDATE purchases SET purchase_number=? WHERE purchase_id=?",
            );
            $number->bind_param("si", $purchase_number, $purchase_id);
            ...
            if ($is_paid) {
                $pay = $conn->prepare(
                    "INSERT INTO purchase_payments(purchase_id,payment_date,amount,payment_method,reference_no,notes,added_by) VALUES(?,?,?,?,?, ?,?)",
                );
                $pay_note =
                    "Payment recorded when pending invoice was completed";
                $pay->bind_param(
                    "isdsssi",
                    $purchase_id,
                    $invoice_date,
            ...
            $product->bind_param("di", $unit_cost, $product_id);
            $product->execute();
            $product->close();
            $finish = $conn->prepare(
                "UPDATE stock_adjustments SET unit_cost=?,total_cost=?,invoice_status='finalized',supplier_id=?,purchase_id=?,supplier_invoice=?,invoice_date=?,finalized_at=NOW(),note=CONCAT(COALESCE(note,''),' | Invoice finalized: ',?) WHERE adjustment_id=?",
            );
            $finish->bind_param(
                "ddiisssi",
                $unit_cost,
            ...
            $conn->commit();
            $msg =
                "Invoice completed as $purchase_number. Stock quantity was not changed; Rs. " .
                number_format($total, 2) .
                " was added to purchase expenses.";
            $msg_type = "success";
        } catch (Throwable $e) {
            $conn->rollback();
            $msg = "Invoice could not be completed: " . $e->getMessage();
            $msg_type = "error";
        }
    }
}
...
if (isset($_GET["toggle"])) {
    $id = (int) $_GET["toggle"];
    $conn->query(
        "UPDATE products SET status=IF(status=1,0,1) WHERE product_id=$id",
    );
...
if ($filter_stat !== "") {
    $where[] = "p.status = " . (int) $filter_stat;
}
...
$supplier_result = $conn->query(
    "SELECT supplier_id,supplier_code,supplier_name,phone FROM suppliers WHERE status=1 ORDER BY supplier_name ASC",
);
if ($supplier_result) {
...
$stock_history = $conn->query(
    "SELECT sa.*,p.product_name,u.full_name FROM stock_adjustments sa JOIN products p ON p.product_id=sa.product_id LEFT JOIN users u ON u.user_id=sa.user_id ORDER BY sa.adjustment_id DESC LIMIT 8",
);
$pending_invoices = $conn->query(
    "SELECT sa.*,p.product_name,p.unit,u.full_name received_by_name FROM stock_adjustments sa JOIN products p ON p.product_id=sa.product_id LEFT JOIN users u ON u.user_id=sa.user_id WHERE sa.adjustment_type='stock_in' AND sa.invoice_status='pending' ORDER BY sa.created_at ASC",
);

/* Counts */
$total_active = $conn
    ->query("SELECT COUNT(*) AS v FROM products WHERE status=1")
    ->fetch_assoc()["v"];
$total_inactive = $conn
    ->query("SELECT COUNT(*) AS v FROM products WHERE status=0")
    ->fetch_assoc()["v"];
$total_all = $total_active + $total_inactive;
$total_stock = (float) $conn
    ->query("SELECT COALESCE(SUM(stock_qty),0) AS v FROM products")
    ->fetch_assoc()["v"];
$low_stock = (int) $conn
    ->query(
        "SELECT COUNT(*) AS v FROM products WHERE stock_qty<=reorder_level AND status=1",
    )
    ->fetch_assoc()["v"];
$inventory_cost = (float) $conn
    ->query("SELECT COALESCE(SUM(cost_price*stock_qty),0) AS v FROM products")
    ->fetch_assoc()["v"];
$potential_profit = (float) $conn
    ->query(
        "SELECT COALESCE(SUM((price-cost_price)*stock_qty),0) AS v FROM products",
    )
    ->fetch_assoc()["v"];
?>
<!DOCTYPE html>
...
<title>Inventory — ST Pvt Ltd.</title>
...
<div class="content">
    ...
    <div class="stat-strip">
        ...
        <div class="stat-tile">
            ...
            <div><div class="st-val"><?php echo number_format(
                $total_stock,
                0,
            ); ?></div><div class="st-lbl">Total Units in Stock</div></div>
        </div>
        <div class="stat-tile">
            <div class="st-icon" style="background:var(--amber-lt);color:var(--amber);"><i class="fa-solid fa-wallet"></i></div>
            <div><div class="st-val" style="font-size:16px;">Rs. <?php echo number_format(
                $inventory_cost,
                0,
            ); ?></div><div class="st-lbl">Inventory Cost</div></div>
        </div>
        <div class="stat-tile">
            <div class="st-icon" style="background:var(--green-lt);color:var(--green);"><i class="fa-solid fa-arrow-trend-up"></i></div>
            <div><div class="st-val" style="font-size:16px;color:var(--green);">Rs. <?php echo number_format(
                $potential_profit,
                0,
            ); ?></div><div class="st-lbl">Potential Retail Profit</div></div>
        </div>
    </div>
    ...
    <section class="card" style="margin-bottom:18px;border:1.5px solid #f6c56f">
        ...
        <?php while ($pi = $pending_invoices->fetch_assoc()): ?>
            <form method="post" class="supplier-form" style="border:1px solid var(--border);border-radius:10px;padding:13px;background:#fafcfd">
                <input type="hidden" name="adjustment_id" value="<?php echo (int) $pi[
                    "adjustment_id"
                ]; ?>">
                <input type="hidden" name="pending_supplier_id" class="supplier-id" value="">
                <div style="display:flex;justify-content:space-between;gap:12px;margin-bottom:10px;flex-wrap:wrap"><strong><?php echo htmlspecialchars(
                    $pi["product_name"],
     ...
     $pi["unit"],
 ); ?></strong><span class="muted">Received <?php
echo date("d M Y, h:i A", strtotime($pi["created_at"]));
if ($pi["note"]): ?> · <?php echo htmlspecialchars($pi["note"]);endif;
?></span></div>
                <div style="display:grid;grid-template-columns:repeat(4,minmax(130px,1fr));gap:9px">
                    ...
                    <div class="field"><label>Invoice Date *</label><input class="inp" type="date" name="pending_invoice_date" value="<?php echo date(
                        "Y-m-d",
                    ); ?>" required></div>
    ...
    <section class="card stock-manager collapsible-panel <?php echo isset(
        $_POST["adjust_stock"],
    )
        ? "panel-open"
        : ""; ?>" id="stockPanel">
        ...
        <div class="card-body">
            <form method="POST" class="stock-form">
                <div class="field product-choice"><label>Product</label><select name="stock_product_id" class="inp" required><option value="">Select product</option><?php while (
                    $sp = $stock_products->fetch_assoc()
                ): ?><option value="<?php echo (int) $sp[
    "product_id"
]; ?>"><?php echo htmlspecialchars(
    $sp["product_name"],
); ?> — <?php echo number_format($sp["stock_qty"], 0) .
     " " .
     htmlspecialchars($sp["unit"]); ?></option><?php endwhile; ?></select></div>
        ...
        <?php if (
            $stock_history &&
            $stock_history->num_rows
        ): ?><div class="stock-history"><div class="stock-history-title">Recent stock adjustments</div><div class="history-list"><?php while (
    $sh = $stock_history->fetch_assoc()
):
    $delta =
        (float) $sh["stock_after"] -
        (float) $sh[
            "stock_before"
        ]; ?><div class="history-chip"><strong><?php echo htmlspecialchars(
    $sh["product_name"],
); ?></strong><span class="<?php echo $delta >= 0 ? "plus" : "minus"; ?>"><?php
echo $delta >= 0 ? "+" : "";
echo number_format($delta, 0);
?></span> · now <?php echo number_format($sh["stock_after"], 0); ?><br><?php
echo date("d M, h:i A", strtotime($sh["created_at"]));
if ($sh["note"]): ?> · <?php echo htmlspecialchars($sh["note"]);endif;
?></div><?php
endwhile; ?></div></div><?php endif; ?>
    </section>
    ...
    <div class="inventory-stack">
        ...
        <!-- ═══ FORM PANEL ═══ -->
        <div class="card form-card collapsible-panel <?php echo $edit_row ||
        isset($_POST["add_product"])
            ? "panel-open"
            : ""; ?>" id="productFormCard">
            <div class="card-head">
                <h4>
                    <i class="fa-solid <?php echo $edit_row
                        ? "fa-pen"
                        : "fa-plus-circle"; ?>"></i>
                    <?php echo $edit_row
                        ? "Edit Product"
            ...
            <div class="card-body">
                <form method="POST" class="product-form supplier-form">
                    <?php if ($edit_row): ?>
                        <input type="hidden" name="edit_id" value="<?php echo $edit_row[
                            "product_id"
                        ]; ?>">
                    <?php endif; ?>
                    ...
                    <?php if (!$edit_row): ?>
                    <div class="form-section-title">1. Supplier &amp; Purchase <small>The supplier is reused if its code or name already exists.</small><label style="display:flex;align-items:center;gap:7px;margin-top:7px;text-transform:none;letter-spacing:0"><input type="checkbox" id="newProductInvoicePending" name="new_product_invoice_pending" value="1" onchange="toggleNewProductInvoice()" <?php echo isset(
                        $_POST["new_product_invoice_pending"],
                    )
                        ? "checked"
                        : ""; ?>> Receive stock now — supplier invoice/details will be added later</label></div>
                    <input type="hidden" name="supplier_id" class="supplier-id" value="<?php echo (int) ($_POST[
                        "supplier_id"
                    ] ?? 0); ?>">
                    ...
                    <div class="field"><label>&nbsp;</label><button type="button" class="btn-secondary" onclick="clearSupplierSelection(this)" style="height:44px"><i class="fa-solid fa-user-plus"></i> New Supplier</button></div>
                    <div class="field"><label>Supplier Code</label><input class="inp supplier-code" name="supplier_code" placeholder="SUP-001" value="<?php echo htmlspecialchars(
                        $_POST["supplier_code"] ?? "",
                    ); ?>"></div>
                    <div class="field span-2"><label>Supplier Name <span id="supplierRequiredMark">*</span></label><input class="inp supplier-name" id="newProductSupplierName" name="supplier_name" placeholder="e.g. Cammy" required value="<?php echo htmlspecialchars(
                        $_POST["supplier_name"] ?? "",
                    ); ?>"></div>
                    <div class="field"><label>Supplier Phone</label><input class="inp supplier-phone" name="supplier_phone" placeholder="071 234 5678" value="<?php echo htmlspecialchars(
                        $_POST["supplier_phone"] ?? "",
                    ); ?>"></div>
                    <div class="field"><label>Supplier Invoice</label><input class="inp" name="supplier_invoice" placeholder="INV-1001" value="<?php echo htmlspecialchars(
                        $_POST["supplier_invoice"] ?? "",
                    ); ?>"></div>
                    <div class="field"><label>Purchase Date *</label><input class="inp" type="date" name="purchase_date" required value="<?php echo htmlspecialchars(
                        $_POST["purchase_date"] ?? date("Y-m-d"),
                    ); ?>"></div>
                    <div class="field"><label>Purchase Quantity *</label><input class="inp" type="number" name="stock_qty" step="0.001" min="0.001" required value="<?php echo htmlspecialchars(
                        $_POST["stock_qty"] ?? "1",
                    ); ?>"></div>
                    ...
                    <div class="field"><label>Cost / Buying Price (Rs.)</label><input class="inp" id="costPrice" type="number" name="cost_price" step="0.01" min="0" placeholder="0.00" value="<?php echo htmlspecialchars(
                        $edit_row["cost_price"] ?? "0.00",
                    ); ?>" oninput="calculateProductProfit()"></div>
                    ...
                    <div class="field"><label>Wholesale Price (Rs.)</label><input class="inp" id="wholesalePrice" type="number" name="wholesale_price" step="0.01" min="0" placeholder="0.00" value="<?php echo htmlspecialchars(
                        $edit_row["wholesale_price"] ?? "0.00",
                    ); ?>" oninput="calculateProductProfit()"></div>
                    <?php if (
                        $edit_row
                    ): ?><div class="field"><label>Current Stock</label><input class="inp" type="number" name="stock_qty" step="0.001" min="0" value="<?php echo htmlspecialchars(
    $edit_row["stock_qty"],
); ?>" readonly title="Use Correct Existing Stock to change stock"></div><?php endif; ?>

                    <!-- Status -->
                    <div class="field">
                        <label>Status</label>
                        <select name="status" class="inp" style="padding-left:14px;">
                            <option value="1" <?php echo !$edit_row ||
                            $edit_row["status"] == 1
                                ? "selected"
                                : ""; ?>>
                                Active (visible on POS)
                            </option>
                            <option value="0" <?php echo $edit_row &&
                            $edit_row["status"] == 0
                                ? "selected"
                                : ""; ?>>
                    ...
                    <details class="advanced-box" open>
                        ...
                        <div class="advanced-fields">
                            <div class="field"><label>Item Code / SKU</label><input class="inp" name="sku" placeholder="SG-001" value="<?php echo htmlspecialchars(
                                $edit_row["sku"] ?? ($_POST["sku"] ?? ""),
                            ); ?>"></div>
                            <div class="field"><label>Barcode</label><input class="inp" name="barcode" placeholder="Scan or type" value="<?php echo htmlspecialchars(
                                $edit_row["barcode"] ??
                                    ($_POST["barcode"] ?? ""),
                            ); ?>"></div>
                            <div class="field"><label>Serial Number</label><input class="inp" name="serial_no" placeholder="Optional" value="<?php echo htmlspecialchars(
                                $edit_row["serial_no"] ??
                                    ($_POST["serial_no"] ?? ""),
                            ); ?>"></div>
                            ...
                            <div class="field"><label>Unit</label><input class="inp" name="unit" placeholder="pcs" value="<?php echo htmlspecialchars(
                                $edit_row["unit"] ?? "pcs",
                            ); ?>"></div>
                            <div class="field"><label>Wholesale Min Qty</label><input class="inp" type="number" name="wholesale_min_qty" min="1" value="<?php echo htmlspecialchars(
                                $edit_row["wholesale_min_qty"] ?? "1",
                            ); ?>"></div>
                            <div class="field"><label>Low-stock Alert</label><input class="inp" type="number" name="reorder_level" step="1" min="0" value="<?php echo htmlspecialchars(
                                $edit_row["reorder_level"] ?? "5",
                            ); ?>"></div>
                        </div>
                    </details>
        ...
        <div style="display:flex;flex-direction:column;gap:12px;">
            ...
            <form method="GET" class="filter-bar">
                <div class="ff">
                    <label>Search</label>
                    <input type="text" name="search" placeholder="Product name…" value="<?php echo htmlspecialchars(
                        $search,
                    ); ?>">
                </div>
                <div class="ff">
                    ...
                    <select name="cat">
                        ...
                        <?php
                        $cat_filter_q = $conn->query(
                            "SELECT * FROM categories ORDER BY category_name ASC",
                        );
                        while ($c = $cat_filter_q->fetch_assoc()) {
                            $sel =
                                $filter_cat == $c["category_id"]
                                    ? "selected"
                                    : "";
                            echo "<option value='{$c["category_id"]}' $sel>" .
                                htmlspecialchars($c["category_name"]) .
                                "</option>";
                        }
                        ?>
                ...
                <div class="ff">
                    ...
                    <select name="stat">
                        ...
                        <option value="1" <?php echo $filter_stat === "1"
                            ? "selected"
                            : ""; ?>>Active</option>
                        <option value="0" <?php echo $filter_stat === "0"
                            ? "selected"
                            : ""; ?>>Inactive</option>
                    </select>
                </div>
            ...
            <div class="card table-card-full">
                <div class="card-head">
                    ...
                    <span class="count-badge"><?php echo $products
                        ? $products->num_rows
                        : 0; ?> found</span>
                </div>
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="color:var(--text-muted);font-size:12px;"><?php echo $row[
                                "product_id"
                            ]; ?></td>
                            <td><span class="cat-chip"><?php echo htmlspecialchars(
                                $row["category_name"],
                            ); ?></span></td>
                            <td>
                            ...
                            <td><div class="price-stack"><small>Cost: Rs. <?php echo number_format(
                                $row["cost_price"],
                                2,
                            ); ?></small><strong>Retail: Rs. <?php echo number_format(
    $row["price"],
    2,
); ?></strong><small>Wholesale: Rs. <?php echo number_format(
    $row["wholesale_price"] > 0 ? $row["wholesale_price"] : $row["price"],
    2,
); ?></small><small style="color:var(--green);">Retail profit: Rs. <?php echo number_format(
    $row["price"] - $row["cost_price"],
    2,
); ?></small></div></td>
                            <td><span class="stock-value <?php echo $row[
                                "stock_qty"
                            ] <= $row["reorder_level"]
                                ? "stock-low"
                                : "stock-ok"; ?>"><i class="fa-solid <?php echo $row[
    "stock_qty"
] <= $row["reorder_level"]
    ? "fa-triangle-exclamation"
    : "fa-cubes"; ?>"></i><?php echo number_format(
    $row["stock_qty"],
    0,
); ?> <?php echo htmlspecialchars($row["unit"]); ?></span></td>
                            <td>
                                <a href="products.php?toggle=<?php echo $row[
                                    "product_id"
                                ]; ?>"
                                   class="status-badge <?php echo $row["status"]
                                       ? "st-active"
                                       : "st-inactive"; ?>">
                                    <i class="fa-solid <?php echo $row["status"]
                                        ? "fa-circle-check"
                                        : "fa-circle-xmark"; ?>"></i>
                                    <?php echo $row["status"]
                                        ? "Active"
                            ...
                            <td>
                                <div class="action-btns">
                                    ...
                                    <a href="products.php?delete=<?php echo $row[
                                        "product_id"
                                    ]; ?>"
                                       class="btn-del"
                                       onclick="return confirm('Delete \'<?php echo addslashes(
                                           $row["product_name"],
                                       ); ?>\'?')">
...
<?php foreach ($supplier_options as $supplier_option):
    $supplier_label = implode(
        " · ",
        array_filter([
            $supplier_option["supplier_code"],
            $supplier_option["supplier_name"],
            $supplier_option["phone"],
        ]),
    ); ?>
...
<script>
const supplierDirectory=<?php echo json_encode(
    $supplier_options,
...
<?php if (
    $edit_row
): ?>setTimeout(()=>document.getElementById('productFormCard')?.scrollIntoView({behavior:'smooth',block:'start'}),100);<?php endif; ?>
</script>
