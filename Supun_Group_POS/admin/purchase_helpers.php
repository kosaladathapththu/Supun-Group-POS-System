...
{
    $stmt = $conn->prepare(
        "SELECT total_amount,COALESCE((SELECT SUM(amount) FROM purchase_payments WHERE purchase_id=?),0) paid FROM purchases WHERE purchase_id=?",
    );
...
function receivePurchase(mysqli $conn, int $purchaseId, int $userId): void
{
    ...
    try {
        $purchaseStmt = $conn->prepare(
            "SELECT status FROM purchases WHERE purchase_id=? FOR UPDATE",
        );
        ...
        $itemsStmt = $conn->prepare(
            "SELECT purchase_item_id,product_id,quantity,unit_cost FROM purchase_items WHERE purchase_id=?",
        );
        $itemsStmt->bind_param("i", $purchaseId);
        $itemsStmt->execute();
        $items = $itemsStmt->get_result();
        if ($items->num_rows === 0) {
            throw new RuntimeException("Purchase has no products.");
        }
        ...
        while ($item = $items->fetch_assoc()) {
            ...
            if (!$product) {
                throw new RuntimeException(
                    "A purchase product no longer exists.",
                );
            }
            ...
            $newCost =
                $after > 0
                    ? ($before * $oldCost + $quantity * $unitCost) / $after
                    : $unitCost;

            $updateProduct = $conn->prepare(
                "UPDATE products SET stock_qty=?,cost_price=? WHERE product_id=?",
            );
            ...
            $receivedItem = $conn->prepare(
                "UPDATE purchase_items SET received_qty=quantity WHERE purchase_item_id=?",
            );
        ...
        $receive = $conn->prepare(
            "UPDATE purchases SET status='received',received_by=?,received_at=NOW() WHERE purchase_id=?",
        );
