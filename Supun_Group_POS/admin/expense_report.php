...
$revenue = $conn
    ...
    WHERE DATE(created_at) BETWEEN '$from_s' AND '$to_s'",
    )
    ->fetch_assoc()["v"];
...
$all_cats_q = $conn->query(
    "SELECT DISTINCT category FROM expenses ORDER BY category",
);
$all_cats = [];
while ($r = $all_cats_q->fetch_assoc()) {
    $all_cats[] = $r["category"];
}
?>
<!DOCTYPE html>
...
<div class="content">
    ...
    <div class="page-header">
        ...
        <div style="display:flex;gap:8px;" class="no-print">
            <a class="btn-secondary" target="_blank" href="print_report.php?type=expenses&amp;from=<?php echo urlencode(
                $f_from,
            ); ?>&amp;to=<?php echo urlencode(
    $f_to,
); ?>&amp;cat=<?php echo urlencode($f_cat); ?>&amp;pm=<?php echo urlencode(
    $f_pm,
); ?>">
    ...
    : ""; ?>><?php echo $p; ?></option>
                ...
                <div style="align-self:flex-end;display:flex;gap:6px;flex-wrap:wrap;">
                    <a href="?from=<?php echo date(
                        "Y-m-01",
                    ); ?>&to=<?php echo date(
    "Y-m-d",
); ?>" class="btn-secondary" style="font-size:11px;padding:8px 10px;">This Month</a>
                    <a href="?from=<?php echo date(
                        "Y-m-01",
    ...
    "Y-m-t",
    strtotime("last month"),
); ?>" class="btn-secondary" style="font-size:11px;padding:8px 10px;">Last Month</a>
                    <a href="?from=<?php echo date(
                        "Y-01-01",
                    ); ?>&to=<?php echo date(
    "Y-m-d",
); ?>" class="btn-secondary" style="font-size:11px;padding:8px 10px;">This Year</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="kpi-row">
        <div class="kpi-card" style="border-top-color:var(--red);">
            ...
            <div class="kc-val" style="color:var(--red);">Rs. <?php echo number_format(
                $kpi["total"],
                0,
            ); ?></div>
            <div class="kc-lbl">Total Expenses</div>
            <div class="kc-sub"><?php echo $kpi[
                "cnt"
            ]; ?> entries in period</div>
        </div>
        <div class="kpi-card" style="border-top-color:var(--amber);">
            ...
            <div class="kc-val" style="color:var(--amber);">Rs. <?php echo number_format(
                $kpi["avg_amt"],
                0,
            ); ?></div>
            <div class="kc-lbl">Avg per Entry</div>
            <div class="kc-sub">Max: Rs. <?php echo number_format(
                $kpi["max_amt"],
                0,
            ); ?></div>
        </div>
        <div class="kpi-card" style="border-top-color:var(--indigo);">
            ...
            <div class="kc-val" style="color:var(--indigo);">Rs. <?php echo number_format(
                $revenue,
                0,
            ); ?></div>
        ...
        <div class="kpi-card" style="border-top-color:<?php echo $profit >= 0
            ? "var(--green)"
            : "var(--red)"; ?>;">
            <div class="kc-icon" style="background:<?php echo $profit >= 0
                ? "var(--green-lt)"
                : "var(--red-lt)"; ?>;color:<?php echo $profit >= 0
    ? "var(--green)"
    : "var(--red)"; ?>;">
                <i class="fa-solid fa-<?php echo $profit >= 0
                    ? "arrow-trend-up"
                    : "arrow-trend-down"; ?>"></i>
            </div>
            <div class="kc-val" style="color:<?php echo $profit >= 0
                ? "var(--green)"
                : "var(--red)"; ?>;">
                Rs. <?php echo number_format(abs($profit), 0); ?>
            </div>
            <div class="kc-lbl"><?php echo $profit >= 0
                ? "Net Profit"
                : "Net Loss"; ?></div>
    ...
    <div class="pl-box">
        <div class="pl-item">
            ...
            <div class="pl-val" style="color:var(--green);">Rs. <?php echo number_format(
                $revenue,
                2,
            ); ?></div>
        </div>
        <div class="pl-item">
            ...
            <div class="pl-val" style="color:var(--red);">Rs. <?php echo number_format(
                $kpi["total"],
                2,
            ); ?></div>
        </div>
        <div class="pl-item">
            <div class="pl-lbl"><i class="fa-solid fa-<?php echo $profit >= 0
                ? "chart-line"
                : "chart-line-down"; ?>"></i> &nbsp;Net <?php echo $profit >= 0
    ? "Profit"
    : "Loss"; ?></div>
            <div class="pl-val" style="color:<?php echo $profit >= 0
                ? "var(--green)"
                : "var(--red)"; ?>;">
                <?php echo $profit < 0
                    ? "-"
                    : ""; ?>Rs. <?php echo number_format(abs($profit), 2); ?>
            </div>
        </div>
    </div>
    ...
    <div class="three-cols">
        ...
        <div class="card table-card-full">
            ...
            <div class="cat-bar-row">
                ...
                <div class="cbr-label">
                    <div class="cbr-name"><?php echo htmlspecialchars(
                        $cr["category"],
                    ); ?></div>
                ...
                <div class="cbr-amt">Rs.<?php echo number_format(
                    $cr["total"],
                    0,
                ); ?></div>
            </div>
        ...
        <div class="card table-card-full">
            ...
            if (!empty($pm_rows)):
                ...
                foreach ($pm_rows as $pr):
                    $pct =
                        $max_pm > 0
                            ? round(($pr["total"] / $max_pm) * 100)
                            : 0; ?>
            <div class="cat-bar-row">
                <span class="badge <?php echo $pm_bdg[$pr["payment_method"]] ??
                    "b-amber"; ?>">
                    <i class="fa-solid <?php echo $pm_icons[
                        $pr["payment_method"]
                    ] ?? "fa-circle-dot"; ?>"></i>
        ...
        <div class="card table-card-full">
            <div class="card-header">
                ...
                <span class="count-badge"><?php echo count(
                    $daily_rows,
                ); ?> days</span>
            </div>
            ...
            <?php if (!empty($daily_rows)):
                ...
                foreach ($daily_rows as $dr):
                    $pct =
                        $max_day > 0
                            ? round(($dr["total"] / $max_day) * 100)
                            : 0; ?>
            <div class="cat-bar-row">
                <div style="flex-shrink:0;min-width:80px;">
                    <div style="font-size:12px;font-weight:800;color:var(--text);"><?php echo date(
                        "d M",
                        strtotime($dr["day"]),
                    ); ?></div>
                    <div style="font-size:10px;color:var(--text-muted);"><?php echo date(
                        "D",
                        strtotime($dr["day"]),
                    ); ?></div>
                </div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><strong style="color:var(--text);"><?php echo htmlspecialchars(
                            $ex["title"],
                        ); ?></strong></td>
                        <td><span class="amt-badge">Rs. <?php echo number_format(
                            $ex["amount"],
                            2,
                        ); ?></span></td>
                        <td>
