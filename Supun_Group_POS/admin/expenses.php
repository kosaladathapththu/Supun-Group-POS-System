...
if (isset($_GET["edit"])) {
    $eid = (int) $_GET["edit"];
    $edit_row = $conn
        ->query("SELECT * FROM expenses WHERE expense_id=$eid")
        ->fetch_assoc();
}
...
if ($f_month) {
    $where[] =
        ...
        $conn->real_escape_string($f_month) .
        "'";
}
if ($f_cat) {
    $where[] = "category='" . $conn->real_escape_string($f_cat) . "'";
}
if ($f_pm) {
    $where[] = "payment_method='" . $conn->real_escape_string($f_pm) . "'";
}
...
$kpi = $conn
    ->query(
        "SELECT COALESCE(SUM(amount),0) AS total, COUNT(*) AS cnt FROM expenses WHERE $ws",
    )
    ->fetch_assoc();
$total_ever = $conn
    ->query("SELECT COALESCE(SUM(amount),0) AS v FROM expenses")
    ->fetch_assoc()["v"];
$this_month = $conn
    ->query(
        "SELECT COALESCE(SUM(amount),0) AS v FROM expenses WHERE DATE_FORMAT(expense_date,'%Y-%m')='" .
            date("Y-m") .
            "'",
    )
    ->fetch_assoc()["v"];
...
$purchase_month_sql = $f_month
    ? "AND DATE_FORMAT(COALESCE(sa.invoice_date,DATE(sa.created_at)),'%Y-%m')='" .
        $conn->real_escape_string($f_month) .
        "'"
    : "";
$purchase_filtered = (float) $conn
    ->query(
        "SELECT COALESCE(SUM(sa.total_cost),0) AS v FROM stock_adjustments sa WHERE sa.adjustment_type='stock_in' $purchase_month_sql",
    )
    ->fetch_assoc()["v"];
$purchase_this_month = (float) $conn
    ->query(
        "SELECT COALESCE(SUM(total_cost),0) AS v FROM stock_adjustments WHERE adjustment_type='stock_in' AND DATE_FORMAT(COALESCE(invoice_date,DATE(created_at)),'%Y-%m')='" .
            date("Y-m") .
            "'",
    )
    ->fetch_assoc()["v"];
$purchases = $conn->query(
    "SELECT sa.*,p.product_name,p.unit,u.full_name AS added_by_name FROM stock_adjustments sa JOIN products p ON p.product_id=sa.product_id LEFT JOIN users u ON u.user_id=sa.user_id WHERE sa.adjustment_type='stock_in' $purchase_month_sql ORDER BY COALESCE(sa.invoice_date,DATE(sa.created_at)) DESC,sa.adjustment_id DESC",
);
...
");
$cat_data = [];
while ($r = $cat_totals->fetch_assoc()) {
    $cat_data[] = $r;
}
...
while ($r = $db_cats->fetch_assoc()) {
    if (!in_array($r["category"], $all_cats)) {
        $all_cats[] = $r["category"];
    }
}
sort($all_cats);
$report_from = $f_month ? $f_month . "-01" : date("Y-m-01");
$report_to = $f_month ? date("Y-m-t", strtotime($report_from)) : date("Y-m-d");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="content">
    ...
    <div class="page-header">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;justify-content:flex-end;">
            <a target="_blank" href="print_report.php?type=expenses&amp;scope=purchases&amp;from=<?php echo urlencode(
                $report_from,
            ); ?>&amp;to=<?php echo urlencode(
    $report_to,
); ?>" class="btn-secondary"><i class="fa-solid fa-boxes-stacked"></i> Stock Purchases Report</a>
            <a target="_blank" href="print_report.php?type=expenses&amp;scope=operating&amp;from=<?php echo urlencode(
                $report_from,
            ); ?>&amp;to=<?php echo urlencode(
    $report_to,
); ?>" class="btn-secondary"><i class="fa-solid fa-receipt"></i> Other Expenses Report</a>
            <a target="_blank" href="print_report.php?type=expenses&amp;scope=full&amp;from=<?php echo urlencode(
                $report_from,
            ); ?>&amp;to=<?php echo urlencode(
    $report_to,
); ?>" class="btn-primary"><i class="fa-solid fa-chart-pie"></i> Full Outgoing Report</a>
        </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:20px;" class="expense-kpis">
        <div class="stat-tile" style="border-left:4px solid var(--red);">
            ...
            <div>
                <div class="st-val" style="color:var(--red);">Rs. <?php echo number_format(
                    $this_month_outgoing,
                    0,
                ); ?></div>
        ...
        <div class="stat-tile" style="border-left:4px solid var(--amber);">
            ...
            <div>
                <div class="st-val" style="color:var(--amber);">Rs. <?php echo number_format(
                    $kpi["total"],
                    0,
                ); ?></div>
                <div class="st-lbl">Operating Expenses &bull; <?php echo $kpi[
                    "cnt"
                ]; ?> entries</div>
            </div>
        </div>
        <div class="stat-tile" style="border-left:4px solid var(--text-muted);">
            ...
            <div>
                <div class="st-val" style="color:var(--indigo);">Rs. <?php echo number_format(
                    $purchase_filtered,
                    0,
                ); ?></div>
        ...
        <div class="stat-tile" style="border-left:4px solid var(--primary);">
            ...
            <div>
                <div class="st-val" style="color:var(--primary);">Rs. <?php echo number_format(
                    $filtered_outgoing,
                    0,
                ); ?></div>
    ...
    <div class="two-col" id="expenseMainGrid" style="align-items:start;">
        ...
        <div class="card form-sticky">
            <div class="card-header">
                <h3>
                    <i class="fa-solid fa-<?php echo $edit_row
                        ? "pen"
                        : "plus-circle"; ?>"></i>
                    <?php echo $edit_row
                        ? "Edit Expense"
                ...
                <form method="POST">
                    ...
                    <div class="field">
                        ...
                        <textarea name="note" class="inp" style="padding:10px 12px;height:70px;resize:vertical;font-family:'Nunito',sans-serif;"
                                  placeholder="Additional details…"><?php echo htmlspecialchars(
                                      $edit_row["note"] ?? "",
                                  ); ?></textarea>
                    </div>
            ...
            <div style="border-top:1.5px solid var(--border);padding:14px 18px 6px;">
                ...
                <div class="cat-row">
                    ...
                    <div style="text-align:right;flex-shrink:0;">
                        <div style="font-size:12px;font-weight:900;color:var(--red);">Rs.<?php echo number_format(
                            $cd["total"],
                            0,
                        ); ?></div>
                        <div style="font-size:10px;color:var(--text-muted);"><?php echo $cd[
                            "cnt"
                        ]; ?> entries</div>
                    </div>
                </div>
            ...
            <form method="GET" class="filter-bar">
                ...
                <div class="ff">
                    ...
                    <select name="pm">
                        ...
                        <?php foreach (
                            ["Cash", "Card", "Bank Transfer", "Cheque", "Other"]
                            as $p
                        ): ?>
                        <option value="<?php echo $p; ?>" <?php echo $f_pm == $p
    ? "selected"
    : ""; ?>><?php echo $p; ?></option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div class="card table-card-full">
                <div class="card-header">
                    ...
                    <span class="count-badge"><?php echo $kpi[
                        "cnt"
                    ]; ?> entries</span>
                </div>
                <div class="tbl-wrap">
                    <table>
                        ...
                        <?php if ($expenses && $expenses->num_rows > 0):
                            while ($ex = $expenses->fetch_assoc()):
                                ...
                                ?>
                        <tr>
                            <td style="color:var(--text-muted);font-size:12px;"><?php echo $ex[
                                "expense_id"
                            ]; ?></td>
                            <td style="white-space:nowrap;">
                                <strong style="font-size:12.5px;"><?php echo date(
                                    "d M Y",
                                    strtotime($ex["expense_date"]),
                                ); ?></strong><br>
                                <span style="font-size:10px;color:var(--text-muted);"><?php echo date(
                                    "D",
                                    strtotime($ex["expense_date"]),
                                ); ?></span>
                            </td>
                            ...
                            <td>
                                ...
                                <div class="note-text"><?php echo htmlspecialchars(
                                    $ex["note"],
                                ); ?></div>
                                <?php endif; ?>
                            </td>
                            <td>
                                <span class="amt-badge">Rs. <?php echo number_format(
                                    $ex["amount"],
                                    2,
                                ); ?></span>
                            </td>
                            ...
                            <td style="font-size:12px;">
                                <?php echo htmlspecialchars(
                                    $ex["added_by_name"] ?? "System",
                                ); ?>
                            </td>
                            <td>
                                <div class="action-btns">
                                    <a href="expenses.php?edit=<?php echo $ex[
                                        "expense_id"
                                    ]; ?>" class="btn-edit">
                                        <i class="fa-solid fa-pen"></i> Edit
                                    </a>
                                    <a href="expenses.php?delete=<?php echo $ex[
                                        "expense_id"
                                    ]; ?>"
                                       class="btn-del"
    ...
    <section class="card table-card-full" id="stockPurchasesSection" style="margin:0 0 18px;">
        <div class="card-header">
            ...
            <div style="display:flex;align-items:center;gap:8px;"><span class="count-badge"><?php echo $purchases
                ? $purchases->num_rows
                : 0; ?> purchases</span><a href="products.php" class="btn-secondary no-print"><i class="fa-solid fa-plus"></i> Record Purchase</a></div>
        </div>
        ...
        <?php if ($purchases && $purchases->num_rows):
            while ($purchase = $purchases->fetch_assoc()): ?>
            <tr><td style="white-space:nowrap;"><strong><?php echo date(
                "d M Y",
                strtotime($purchase["invoice_date"] ?: $purchase["created_at"]),
            ); ?></strong><br><small style="color:var(--text-muted);"><?php echo $purchase[
    "invoice_status"
] === "finalized"
    ? "Invoice date"
    : date(
    ...
    ); ?></small></td><td><strong><?php echo htmlspecialchars(
    $purchase["product_name"],
); ?></strong></td><td><span class="purchase-qty"><?php echo number_format(
    $purchase["quantity"],
    0,
) .
    " " .
    htmlspecialchars(
        $purchase["unit"],
    ); ?></span></td><td>Rs. <?php echo number_format(
    $purchase["unit_cost"],
    2,
); ?></td><td class="purchase-total">Rs. <?php echo number_format(
    $purchase["total_cost"],
    2,
); ?></td><td><?php echo htmlspecialchars(
    $purchase["note"] ?: "—",
); ?></td><td><?php echo htmlspecialchars(
    $purchase["added_by_name"] ?: "System",
); ?></td></tr>
        <?php endwhile;
        else:
             ?><tr><td colspan="7" class="empty-row"><i class="fa-solid fa-box-open" style="font-size:22px;display:block;margin-bottom:7px;"></i>No stock purchases recorded for this month. Use Stock In from Inventory.</td></tr><?php
        endif; ?>
        </tbody></table></div>
    </section>
