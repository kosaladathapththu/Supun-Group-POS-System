...
function outputProductXlsxTemplate(?mysqli $conn = null): void
{
    $rows = [
    ...
    if ($conn) {
        $categoryResult = $conn->query(
            "SELECT category_name FROM categories WHERE status=1 ORDER BY category_name",
        );
        if ($categoryResult) {
    ...
    foreach ($rows as $ri => $row) {
        ...
        foreach ($row as $ci => $value) {
            $ref = xlsxColumnName($ci) . ($ri + 1);
            $style = $ri === 0 ? ' s="1"' : "";
            if (is_numeric($value) && $ri > 0) {
                $cells .=
                    '<c r="' .
                    $ref .
                    '"' .
    ...
    $categoryLast = max(1, count($categoryNames));
    $categoryFormula = "CategoryList";
    $sheet =
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><cols><col min="1" max="2" width="18" customWidth="1"/><col min="3" max="3" width="42" customWidth="1"/><col min="4" max="16" width="18" customWidth="1"/></cols><sheetData>' .
        $sheetRows .
        '</sheetData><dataValidations count="1"><dataValidation type="list" allowBlank="1" showInputMessage="1" showErrorMessage="0" promptTitle="Choose or type a category" prompt="Select an existing category, or type a new category manually." sqref="L2:L1001"><formula1>' .
        $categoryFormula .
        "</formula1></dataValidation></dataValidations></worksheet>";
    $categoryRows = "";
    foreach ($categoryNames as $ri => $categoryName) {
        $safe = htmlspecialchars(
            (string) $categoryName,
            ENT_XML1 | ENT_QUOTES,
            "UTF-8",
    ...
    $categorySheet =
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><cols><col min="1" max="1" width="35" customWidth="1"/></cols><sheetData>' .
        $categoryRows .
        "</sheetData></worksheet>";
    $tmp = tempnam(sys_get_temp_dir(), "product-template-");
    $zip = new ZipArchive();
    ...
    $zip->addFromString(
        "xl/workbook.xml",
        '<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Products" sheetId="1" r:id="rId1"/><sheet name="Categories" sheetId="2" state="hidden" r:id="rId2"/></sheets><definedNames><definedName name="CategoryList">Categories!$A$1:$A$' .
            $categoryLast .
    ...
    $zip->addFromString("xl/worksheets/sheet2.xml", $categorySheet);
    $zip->close();
    header(
    ...
    try {
        ...
        if ($sheetXml === false) {
            throw new RuntimeException(
                "The workbook does not contain a first worksheet.",
            );
        }
        $dom = new DOMDocument();
        ...
        foreach ($xp->query("//x:sheetData/x:row") as $rowNode) {
            $row = [];
            foreach ($xp->query("./x:c", $rowNode) as $cell) {
                $index = xlsxCellColumn($cell->getAttribute("r"));
                $type = $cell->getAttribute("t");
                $value = "";
                if ($type === "inlineStr") {
                    foreach ($xp->query(".//x:t", $cell) as $t) {
                        $value .= $t->textContent;
                    }
                } else {
                    $v = $xp->query("./x:v", $cell)->item(0);
                    $raw = $v ? $v->textContent : "";
                    $value =
                        $type === "s" && $raw !== ""
                            ? $shared[(int) $raw] ?? ""
...
function readProductCsv(string $path): array
{
    $rows = [];
    $fh = fopen($path, "r");
...
function normalizeProductHeader(string $value): string
{
    return strtolower(
        trim(preg_replace("/[^a-z0-9]+/i", " ", str_replace("_", " ", $value))),
    );
}
...
function mapProductImportRows(array $rows, mysqli $conn): array
{
    ...
    foreach ($required as $name) {
        if (!in_array($name, $headers, true)) {
            throw new RuntimeException(
                "Missing required column: " . ucwords($name),
            );
        }
    }
    $mapped = [];
    ...
    foreach ($rows as $ri => $values) {
        ...
        foreach ($headers as $i => $h) {
            $data[$h] = trim((string) ($values[$i] ?? ""));
        }
        if (
            isset($data["purchase date"]) &&
            is_numeric($data["purchase date"]) &&
            (float) $data["purchase date"] > 20000
        ) {
            $data["purchase date"] = gmdate(
                "Y-m-d",
                (int) (((float) $data["purchase date"] - 25569) * 86400),
            );
        }
        if (implode("", $data) === "") {
            continue;
        }
        $data["wholesale min qty"] = "1";
        $data["reorder level"] = "5";
        $data["status"] = "Active";
        $sku = $data["item code"] ?? "";
        ...
        if ((float) ($data["retail price"] ?? 0) <= 0) {
            $errors[] = "Retail Price must be greater than zero";
        }
        if ((float) ($data["cost price"] ?? 0) < 0) {
            $errors[] = "Cost Price cannot be negative";
        }
        if ((float) ($data["purchase quantity"] ?? 0) <= 0) {
            $errors[] = "Purchase Quantity must be greater than zero";
        }
        if (
            ($data["purchase date"] ?? "") !== "" &&
            strtotime($data["purchase date"]) === false
        ) {
            $errors[] = "Purchase Date is invalid";
        }
        $existingProductId = 0;
        if ($sku !== "") {
            if (isset($seenSku[strtolower($sku)])) {
                $errors[] = "Duplicate Item Code in this file";
            }
            $seenSku[strtolower($sku)] = 1;
            $skuStmt->bind_param("s", $sku);
            $skuStmt->execute();
            $existing = $skuStmt->get_result()->fetch_assoc();
            if ($existing) {
        ...
        if ($barcode !== "") {
            if (isset($seenBarcode[$barcode])) {
                $errors[] = "Duplicate Barcode in this file";
            }
            $seenBarcode[$barcode] = 1;
            $barcodeStmt->bind_param("s", $barcode);
            $barcodeStmt->execute();
            $barcodeProduct = $barcodeStmt->get_result()->fetch_assoc();
            if (
                $barcodeProduct &&
                (int) $barcodeProduct["product_id"] !== $existingProductId
            ) {
                $errors[] = "Barcode belongs to another product";
            }
        }
        $data["_existing_product_id"] = $existingProductId;
