...
function mapProductImportRows(array $rows, mysqli $conn): array {
    ...
    foreach($rows as $ri=>$values){$data=[];foreach($headers as $i=>$h)$data[$h]=trim((string)($values[$i]??''));if(isset($data['purchase date'])&&is_numeric($data['purchase date'])&&(float)$data['purchase date']>20000)$data['purchase date']=gmdate('Y-m-d',(int)(((float)$data['purchase date']-25569)*86400));if(implode('',$data)==='')continue;$sku=$data['item code']??'';$barcode=$data['barcode']??'';$errors=[];
