...
$stmt = $conn->prepare(
    "SELECT o.*, u.full_name FROM orders o LEFT JOIN users u ON o.user_id=u.user_id WHERE o.order_id=? LIMIT 1",
);
...
$stmt = $conn->prepare(
    "SELECT oi.*, COALESCE(p.product_name,oi.custom_item_name,'Custom Item') AS product_name FROM order_items oi LEFT JOIN products p ON oi.product_id=p.product_id WHERE oi.order_id=? ORDER BY oi.order_item_id",
);
$stmt->bind_param("i", $order_id);
$stmt->execute();
...
<section class="card">
 <div class="card-header"><h3><i class="fa-solid fa-circle-info"></i> Sale Summary</h3><span class="badge b-green"><?php echo strtoupper(
     htmlspecialchars($order["payment_status"]),
 ); ?></span></div>
 <div class="card-body sale-summary">
  <div class="sale-meta"><label>Sale Type</label><strong><?php echo ucfirst(
      htmlspecialchars($order["order_type"]),
  ); ?></strong></div>
  <div class="sale-meta"><label>Customer</label><strong><?php echo htmlspecialchars(
      $order["customer_name"] ?: "Walk-in Customer",
  ); ?></strong></div>
  <div class="sale-meta"><label>Cashier</label><strong><?php echo htmlspecialchars(
      $order["full_name"] ?: "Unknown",
  ); ?></strong></div>
  <div class="sale-meta"><label>Payment</label><strong><?php echo htmlspecialchars(
      $order["payment_method"],
  ); ?></strong></div>
  <div class="sale-meta"><label>Date</label><strong><?php echo date(
      "d M Y, h:i A",
      strtotime($order["created_at"]),
  ); ?></strong></div>
  <div class="sale-meta"><label>Total Amount</label><strong class="sale-total">Rs. <?php echo number_format(
      $order["total_amount"],
      2,
  ); ?></strong></div>
 </div>
...
<section class="card items-card">
 ...
 <?php while ($item = $items->fetch_assoc()): ?><tr><td><?php
echo htmlspecialchars($item["product_name"]);
if (!$item["product_id"]): ?><span class="custom-chip">Custom</span><?php endif;
?></td><td class="num"><?php echo htmlspecialchars(
    $item["quantity"],
); ?></td><td class="num">Rs. <?php echo number_format(
    $item["unit_price"],
    2,
); ?></td><td class="num"><strong>Rs. <?php echo number_format(
    $item["line_total"],
    2,
); ?></strong></td></tr><?php endwhile; ?>
 </tbody></table></div>
</section>
</div></div></div>
</body>
