...
if (isset($_POST["add_category"])) {
    ...
    if ($name !== "") {
        $find = $conn->prepare(
            "SELECT category_id FROM categories WHERE LOWER(category_name)=LOWER(?) LIMIT 1",
        );
        ...
        if ($existing) {
            ...
            $add->bind_param("si", $name, $status);
            if ($add->execute()) {
                $msg = "Category added successfully.";
                $msg_type = "success";
            } else {
                $msg = "Category could not be added: " . $add->error;
                $msg_type = "error";
            }
            $add->close();
        }
...
if (isset($_POST["edit_category"])) {
    ...
    if ($name !== "") {
        ...
        try {
            $find = $conn->prepare(
                "SELECT category_id FROM categories WHERE LOWER(category_name)=LOWER(?) AND category_id<>? LIMIT 1",
            );
            $find->bind_param("si", $name, $id);
            $find->execute();
            $duplicate = $find->get_result()->fetch_assoc();
            $find->close();
            if ($duplicate) {
                $target = (int) $duplicate["category_id"];
                $move = $conn->prepare(
                    "UPDATE products SET category_id=? WHERE category_id=?",
                );
                $move->bind_param("ii", $target, $id);
                $move->execute();
                ...
                $activate->bind_param("ii", $status, $target);
                $activate->execute();
                $activate->close();
                $conn->commit();
                $msg = "Categories merged successfully. $moved product(s) moved to '$name'.";
                $msg_type = "success";
            } else {
                $update = $conn->prepare(
                    "UPDATE categories SET category_name=?,status=? WHERE category_id=?",
                );
                $update->bind_param("sii", $name, $status, $id);
                if (!$update->execute()) {
            ...
            $conn->rollback();
            $msg = "Category was not updated: " . $e->getMessage();
            $msg_type = "error";
        }
    } else {
        $msg = "Category name cannot be empty.";
...
if (isset($_GET["delete"])) {
    ...
    try {
        $categoryCheck = $conn->prepare(
            "SELECT category_name FROM categories WHERE category_id=? FOR UPDATE",
        );
        $categoryCheck->bind_param("i", $id);
        $categoryCheck->execute();
        $category = $categoryCheck->get_result()->fetch_assoc();
        $categoryCheck->close();
        if (!$category) {
            throw new RuntimeException("Category no longer exists.");
        }
        if (strcasecmp($category["category_name"], "Uncategorized") === 0) {
            throw new RuntimeException(
                "The Uncategorized category is required as a safe destination and cannot be deleted.",
            );
        }
        $count = $conn->prepare(
        ...
        $count->execute();
        $used = (int) $count->get_result()->fetch_assoc()["total"];
        $count->close();
        if ($used > 0) {
            $findFallback = $conn
                ->query(
                    "SELECT category_id FROM categories WHERE LOWER(category_name)='uncategorized' LIMIT 1",
                )
                ->fetch_assoc();
            if ($findFallback) {
                $fallbackId = (int) $findFallback["category_id"];
            } else {
                $conn->query(
                    "INSERT INTO categories(category_name,status) VALUES('Uncategorized',1)",
                );
                $fallbackId = $conn->insert_id;
            }
            $move = $conn->prepare(
                "UPDATE products SET category_id=? WHERE category_id=?",
            );
            $move->bind_param("ii", $fallbackId, $id);
            if (!$move->execute()) {
        ...
        if (!$delete->execute() || $delete->affected_rows === 0) {
            throw new RuntimeException(
                $delete->error ?: "Category was not deleted.",
            );
        }
        $delete->close();
        $conn->commit();
        $msg =
            $used > 0
                ? "Category deleted successfully. $used product(s) were moved to Uncategorized."
                : "Category deleted successfully.";
        $msg_type = "success";
    } catch (Throwable $e) {
        $conn->rollback();
        $msg = "Category was not deleted: " . $e->getMessage();
        $msg_type = "error";
    }
}
...
if (isset($_GET["toggle"])) {
    $id = (int) $_GET["toggle"];
    $toggle = $conn->prepare(
        "UPDATE categories SET status=IF(status=1,0,1) WHERE category_id=?",
    );
    $toggle->bind_param("i", $id);
    $toggle->execute();
    $toggle->close();
...
if (isset($_GET["edit"])) {
    $eid = (int) $_GET["edit"];
    $edit_row = $conn
        ->query("SELECT * FROM categories WHERE category_id=$eid")
        ->fetch_assoc();
}
...
<div class="content">
  ...
  <div class="two-col">
    ...
    <div class="card form-card">
      <div class="card-head">
        <h4><i class="fa-solid <?php echo $edit_row
            ? "fa-pen"
            : "fa-plus"; ?>"></i>
          ...
          <div class="field">
            ...
            <select name="status" class="inp" style="padding-left:14px;">
              ...
              $edit_row["status"] == 1
                  ? "selected"
                  : ""; ?>>Active</option>
              <option value="0" <?php echo $edit_row && $edit_row["status"] == 0
                  ? "selected"
                  : ""; ?>>Inactive</option>
            </select>
          </div>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <a href="categories.php?toggle=<?php echo $row[
                  "category_id"
    ...
    : "st-inactive"; ?>">
            ...
            <td>
              <div class="action-btns">
                <a href="categories.php?edit=<?php echo $row[
                    "category_id"
                ]; ?>" class="btn-edit"><i class="fa-solid fa-pen"></i> Edit</a>
                <a href="categories.php?delete=<?php echo $row[
                    "category_id"
                ]; ?>" class="btn-del"
