...
if (!$payment) {
    ...
    die("Advance payment receipt not found.");
}
...
if (!empty($payment["order_id"])) {
    ...
    $items = $conn->query(
        "SELECT COALESCE(p.product_name,oi.custom_item_name,'Item') item_name,oi.quantity,oi.price,oi.line_total FROM order_items oi LEFT JOIN products p ON p.product_id=oi.product_id WHERE oi.order_id=$oid ORDER BY oi.order_item_id",
    );
    if ($items) {
...
$bill_total =
    (float) ($payment["order_total"] ?? 0) > 0
        ? (float) $payment["order_total"]
        : $calculated_total;
...
$is_bill_refund =
    $is_refund && ($payment["order_status"] ?? "") === "cancelled";
$is_order_installment =
    $is_deposit && !empty($payment["order_id"]) && $payment_number > 0;
$title = $is_bill_refund
    ? "BILL CANCELLATION & REFUND RECEIPT"
    : ($is_order_installment
        ? strtoupper($payment_ordinal . " PAYMENT RECEIPT")
        : ($is_deposit
            ? "ADVANCE PAYMENT RECEIPT"
            : ($is_refund
                ? "ADVANCE SETTLEMENT RECEIPT"
                : "ADVANCE USAGE RECEIPT")));
$seal_main = $is_bill_refund
    ? "Bill Cancelled"
    : ($is_refund
        ? "Advance Settled"
        : ($is_order_installment
            ? $payment_ordinal . " Payment"
            : "Advance Payment"));
$seal_small = $is_refund
    ? "Refunded"
    : ($is_deposit
        ? ($is_order_installment
...
<div class="seal-wrap"><div class="advance-seal"><strong><?php echo htmlspecialchars(
    $seal_main,
...
<table class="meta">
<tr><td>Receipt No.</td><td>: <?php echo htmlspecialchars(
    $payment["receipt_number"],
); ?></td></tr>
<tr><td>Date / Time</td><td>: <?php echo date(
    "d M Y, h:i A",
    strtotime($payment["created_at"]),
); ?></td></tr>
<tr><td>Account No.</td><td>: <?php echo htmlspecialchars(
    $payment["account_number"],
); ?></td></tr>
<tr><td>Customer</td><td>: <?php echo htmlspecialchars(
    $payment["customer_name"],
); ?></td></tr>
<?php if (
    $payment["phone"]
): ?><tr><td>Phone</td><td>: <?php echo htmlspecialchars(
    $payment["phone"],
); ?></td></tr><?php endif; ?>
<tr><td>Payment Method</td><td>: <?php echo htmlspecialchars(
    $payment["payment_method"],
); ?></td></tr>
<tr><td>Received By</td><td>: <?php echo htmlspecialchars(
    $payment["cashier_name"] ?? "-",
); ?></td></tr>
<?php if (
    $payment["order_number"]
): ?><tr><td>Order</td><td>: <?php echo htmlspecialchars(
    $payment["order_number"],
); ?></td></tr><?php endif; ?>
<?php if (
    $payment_number > 0
): ?><tr><td>Payment No.</td><td>: <?php echo $payment_number; ?></td></tr><?php endif; ?>
</table>
<?php if (
    $order_items
): ?><div class="section-title">ITEMS / PRODUCTS</div><table class="items"><thead><tr><th>Item</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead><tbody><?php foreach (
    $order_items
    as $item
    ...
    (float) $item["quantity"],
    (float) $item["quantity"] == (int) $item["quantity"] ? 0 : 2,
); ?></td><td><?php echo number_format(
    (float) $item["price"],
    2,
); ?></td><td><?php echo number_format(
...
<?php if (
    $payment["order_id"]
): ?><div class="balance"><span>Bill Total</span><span>Rs. <?php echo number_format(
    $bill_total,
    2,
); ?></span></div><?php endif; ?>
<div class="amount-box"><small><?php echo $is_order_installment
    ? "INSTALLMENT AMOUNT RECEIVED"
    : ($is_deposit
        ? "ACCOUNT CREDIT RECEIVED"
        : ($is_refund
            ? "AMOUNT REFUNDED"
            : "ACCOUNT CREDIT USED")); ?></small><strong>Rs. <?php echo number_format(
    (float) $payment["amount"],
    2,
); ?></strong></div>
<?php if (
    $payment_history
): ?><div class="section-title">PAYMENT HISTORY</div><table class="payments"><?php foreach (
    $payment_history
    as $index => $paid
    ...
    date(
        "d M Y",
        strtotime($paid["created_at"]),
    ); ?><br><small><?php echo htmlspecialchars(
    $paid["receipt_number"] . " · " . $paid["payment_method"],
); ?></small></td><td>Rs. <?php echo number_format(
    (float) $paid["amount"],
    2,
); ?></td></tr><?php endforeach; ?></table><?php endif; ?>
<?php if (
    $is_bill_refund
): ?><div class="balance"><span>Original Bill Total</span><span>Rs. <?php echo number_format(
    $bill_total,
    2,
); ?></span></div><div class="balance due" style="background:#fef3f2;color:#b42318"><span>Bill Status</span><span>CANCELLED &amp; REFUNDED</span></div>
<?php elseif (
    $is_order_installment
): ?><div class="balance"><span>Total Installments Paid</span><span>Rs. <?php echo number_format(
    $total_paid,
    2,
); ?></span></div><div class="balance due"><span>Bill Balance Remaining</span><span>Rs. <?php echo number_format(
    $remaining_balance,
    2,
); ?></span></div>
<?php elseif (
    $is_deposit
): ?><div class="balance due"><span>Available Account Credit</span><span>Rs. <?php echo number_format(
    (float) $payment["advance_balance"],
    2,
); ?></span></div><?php endif; ?>
<?php if (
    $payment["reference_note"]
): ?><div class="note"><strong>Reference:</strong> <?php echo htmlspecialchars(
    $payment["reference_note"],
); ?></div><?php endif; ?>
<div class="divider"></div><div class="footer"><?php echo $is_bill_refund
    ? "This receipt confirms the bill cancellation and customer refund."
    : ($is_order_installment
        ? "This receipt confirms an installment assigned only to the stated order."
        : "This receipt confirms flexible customer account credit for a future purchase."); ?><br>Thank you for your business.</div>
</section><div class="format-picker"><label for="printFormat">Paper size</label><select id="printFormat" onchange="setPrintFormat(this.value)"><option value="80">Thermal 80mm</option><option value="58">Thermal 58mm</option><option value="a4">A4 Page</option></select></div><div class="actions"><button class="print" onclick="window.print()">Print Advance Bill</button><a class="back" href="<?php echo htmlspecialchars(
    $back_url,
); ?>">&larr; <?php echo htmlspecialchars($back_label); ?></a></div></main>
