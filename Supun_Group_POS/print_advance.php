...
<div class="amount-box"><small><?php echo $is_order_installment?'INSTALLMENT AMOUNT RECEIVED':($is_deposit?'ACCOUNT CREDIT RECEIVED':($is_refund?'AMOUNT REFUNDED':'ACCOUNT CREDIT USED')); ?></small><strong>Rs. <?php echo number_format((float)$payment['amount'],2); ?></strong></div>
...
<?php elseif($is_order_installment): ?><div class="balance"><span>Total Installments Paid</span><span>Rs. <?php echo number_format($total_paid,2); ?></span></div><div class="balance due"><span>Bill Balance Remaining</span><span>Rs. <?php echo number_format($remaining_balance,2); ?></span></div>
<?php elseif($is_deposit): ?><div class="balance due"><span>Available Account Credit</span><span>Rs. <?php echo number_format((float)$payment['advance_balance'],2); ?></span></div><?php endif; ?>
...
<div class="divider"></div><div class="footer"><?php echo $is_bill_refund?'This receipt confirms the bill cancellation and customer refund.':($is_order_installment?'This receipt confirms an installment assigned only to the stated order.':'This receipt confirms flexible customer account credit for a future purchase.'); ?><br>Thank you for your business.</div>
