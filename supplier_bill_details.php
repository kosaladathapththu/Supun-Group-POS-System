...
$stmt=$db->prepare('SELECT id,payment_no,payment_date,amount,method,reference_no,notes,status,reversal_type,reversal_reason,reversed_at FROM supplier_payments WHERE purchase_id=? ORDER BY payment_date,id');$stmt->execute([$id]);$payments=$stmt->fetchAll();
